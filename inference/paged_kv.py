...
class PageAllocator:
    ...
    def alloc(self) -> int:
        if not self.free:
            raise MemoryError("PagedKV: out of free pages")
...
def truncate_state(
    ...
    """
    Truncate KV state to new_len tokens.
    Free any pages no longer needed.
    """
    if new_len > state.seq_len:
        raise ValueError("truncate_state: new_len > current length")
...
def gather_kv(
    ...
    needed_pages = (upto_len + page_size - 1) 
    ks = []
    vs = []
